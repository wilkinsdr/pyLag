...
class MLFit(object):
    ...
    def __init__(self, t, y, noise=None, psdnorm=1., Nf=10, fbins=None, model=None, component_name=None, model_args={}, eval_sin=False, extend_freq=None):
        ...
        self.tau = self.time - jnp.expand_dims(self.time, 1)

        if fbins is None:
            # set up frequency bins to span
            min_freq = (1. / (jnp.max(self.time) - jnp.min(self.time))) / extend_freq
        ...
        if extend_freq is not None:
            ...
            self.fbins = Binning(bin_edges=jnp.insert(self.fbins.bin_edges, 0, extend_freq*self.fbins.bin_edges.min()))
        ...
        c = np.zeros((len(self.fbins), self.tau.shape[0], self.tau.shape[1]))
        s = np.zeros((len(self.fbins), self.tau.shape[0], self.tau.shape[1])) if eval_sin else None
        diag = np.eye(self.tau.shape[0], dtype=bool)
        for i, (fmin, fmax) in enumerate(zip(self.fbins.bin_start, self.fbins.bin_end)):
            c[i, ~diag] = (1. / (2. * np.pi * self.tau[~diag])) * (
                        np.sin(2. * np.pi * fmax * self.tau[~diag]) - np.sin(2. * np.pi * fmin * self.tau[~diag]))
            c[i, diag] = fmax - fmin
            if eval_sin:
                s[i, ~diag] = (1. / (2. * np.pi * self.tau[~diag])) * (
                            np.cos(2. * np.pi * fmax * self.tau[~diag]) - np.cos(2. * np.pi * fmin * self.tau[~diag]))
        self.cos_integral = jnp.array(c)
        self.sin_integral = jnp.array(s)
    ...
    def log_likelihood(self, param_arr, eval_gradient=True):
        """
        ...
        try:
            ...
            try:
                ...
                L = cho_factor(c + jnp.diag(self.noise), lower=True, check_finite=False)[0]
            except jnp.linalg.LinAlgError:
                #printmsg(2, "WARNING: Couldn't invert covariance matrix with parameters " + param2array(params))
                return (-1e6, jnp.zeros(len([p for p in params if params[p].vary])) - 1e6) if eval_gradient else -1e6
        except ValueError:
            return (jnp.inf, jnp.zeros(len([p for p in params if params[p].vary]))) if eval_gradient else -jnp.inf

        alpha = cho_solve((L, True), self.data, check_finite=False)

        log_likelihood_dims = -0.5 * jnp.einsum("ik,ik->k", self.data, alpha)
        log_likelihood_dims -= jnp.log(jnp.diag(L)).sum()
        log_likelihood_dims -= c.shape[0] / 2 * jnp.log(2 * jnp.pi)
        log_likelihood = log_likelihood_dims.sum(-1)

        if eval_gradient:
            c_gradient = self.cov_matrix_deriv(param_arr)
            tmp = jnp.einsum("ik,jk->ijk", alpha, alpha)
            tmp -= cho_solve((L, True), jnp.eye(c.shape[0]))[:, :, jnp.newaxis]
            gradient_dims = 0.5 * jnp.einsum("ijl,ijk->kl", tmp, c_gradient)
    ...
    def _dofit(self, init_params, method='BFGS', **kwargs):
        """
        ...
        """
        initial_par_arr = jnp.array([init_params[p].value for p in init_params])
        ...
        def objective(par_arr):
            ...
            """
            l = self.log_likelihood(par_arr, eval_gradient=False)
            #print("\r-log(L) = %6.3g" % l + " for parameters: " + ' '.join(['%6.3g' % p for p in param2array(fit_params)]), end="")
            return -l

        result = minimize(objective, initial_par_arr, method=method, **kwargs)
    ...
    def fit(self, init_params=None, update_params=True, **kwargs):
        ...
        if True or self.fit_result.success and update_params:
            for par, value in zip([p for p in init_params], self.fit_result.x):
                self.params[par].value = value

            hess = self.fit_result.hess_inv(self.fit_result.x) if callable(self.fit_result.hess_inv) else jnp.diag(self.fit_result.hess_inv)
    ...
    def process_mcmc(self, mcmc_result):
        maxprob = jnp.argmax(mcmc_result.lnprob)
        for p in self.params:
            self.params[p].value = mcmc_result.flatchain[p][maxprob]
        self.param_error = jnp.array([jnp.percentile(mcmc_result.flatchain[p], [15.9, 84.1]) for p in self.params])
    ...
    def nested_sample(self, params=None, prior_fn=None, log_dir=None, resume=True, frac_remain=None, step='adaptive', **kwargs):
        ...
        if prior_fn is None:
            def prior_fn(quantiles):
                ...
                return jnp.array([uniform_dist(q, params[p].max, params[p].min) for q, p in zip(quantiles, var_params)])
...
class MLPSD(MLFit):
    ...
    def __init__(self, lc=None, t=None, r=None, e=None, noise='errors', **kwargs):
        ...
        dt = jnp.min(jnp.diff(t))
        #length = (jnp.max(t) - jnp.min(t)) // dt
        length = len(r)

        if noise == 'poisson':
            noise_arr = (1. / lc.mean()**2) * jnp.ones_like(lc.rate)
        elif noise == 'errors':
            # Note: RMS normalisation
            #noise_arr = e ** 2 * length / (2.*dt)
            noise_arr = e**2
        elif isinstance(noise, (float, int)):
            noise_arr = noise * jnp.ones_like(lc.rate)
        else:
            noise_arr = None

        # RMS normalisation for PSD
        #psdnorm = (jnp.mean(r) ** 2 * length) / (2 * dt)
        psdnorm = jnp.mean(r)**2
        ...
        self.noise_level = (2. / jnp.mean(r)**2) * jnp.ones_like(self.fbins.bin_cent)

    def cov_matrix(self, param_arr):
        """
        ...
        # note the factor of 2 to integrate over the negative frequencies too!
        if self.model is None:
            psd = jnp.exp(param_arr) * self.psdnorm
        else:
            psd = self.model(param_arr, self.fbins.bin_cent) * self.psdnorm

        cov = jnp.sum(jnp.array([p * c for p, c in zip(psd, self.cos_integral)]), axis=0)

        return cov

    def cov_matrix_deriv(self, param_arr):
        """
        ...
        if self.model is None:
            ...
            return jnp.stack([c * p for c, p in zip(self.cos_integral, psd)], axis=-1)
        else:
            psd_deriv = self.model.eval_gradient(params, self.fbins.bin_cent) * self.psdnorm
            return jnp.stack([jnp.sum([c * p for c, p in zip(self.cos_integral, psd_deriv[:, par])], axis=0) for par in range(psd_deriv.shape[-1])], axis=-1)
    ...
    def get_psd(self, params=None):
        ...
        if self.model is None:
            return jnp.array([self.params[p].value for p in self.params])
        else:
            return jnp.log(self.model(self.params, self.fbins.bin_cent))
    ...
    def process_fit_results(self, fit_result, params):
        ...
        if self.model is None:
            ...
            self.psd_error = jnp.sum([e * psd_deriv[..., i] for i, e in enumerate(self.param_error)], axis=0) / self.psd
            if jnp.any(jnp.isnan(self.psd_error)):
                self.psd_error = None

    def _getplotdata(self):
        x = (self.fbins.bin_cent, self.fbins.x_error())
        y = (jnp.exp(self.psd), jnp.vstack([jnp.exp(self.psd) - jnp.exp(self.psd - self.psd_error), jnp.exp(self.psd + self.psd_error) - jnp.exp(self.psd)])) if self.psd_error is not None else jnp.exp(self.psd)
        return x, y
...
class MLCrossSpectrum(MLFit):
    ...
    def __init__(self, lc1, lc2, mlpsd1=None, mlpsd2=None, psd_model=None, cpsd_model=None, lag_model=None, freeze_psd=True, noise='errors', cpsd_model_args={}, lag_model_args={}, **kwargs):
        t = jnp.array(lc1.time)

        r1 = jnp.array(lc1.rate)
        r2 = jnp.array(lc2.rate)

        e1 = jnp.array(lc1.error)
        e2 = jnp.array(lc2.error)
        ...
        nonzero = jnp.logical_and(r1>0, r2>0)
        t = t[nonzero]
        r1 = r1[nonzero]
        r2 = r2[nonzero]
        e1 = e1[nonzero]
        ...
        #cpsdnorm = (lc1.mean() * lc2.mean() * lc1.length) / (2 * jnp.min(jnp.diff(lc1.time)))
        cpsdnorm = jnp.mean(r1) * jnp.mean(r2)
        ...
        noise_arr = jnp.concatenate([self.mlpsd1.noise, self.mlpsd1.noise])
    ...
    def get_params(self):
        ...
        if self.lag_model is None:
            for i in range(len(self.fbins)):
                params.add('%slag%01d' % (self.prefix, i), 0., vary=True, min=-jnp.pi, max=jnp.pi)
        else:
    ...
    def cross_cov_matrix(self, params):
        ...
        # if no model is specified, the PSD model is just the PSD value in each frequency bin
        if self.cpsd_model is None:
            cpsd = jnp.exp(jnp.array([params['%sln_cpsd%01d' % (self.prefix, i)].value for i in range(len(self.fbins))])) * self.psdnorm
        else:
            cpsd = self.cpsd_model(params, self.fbins.bin_cent) * self.psdnorm

        # likewise for the (phase) lags
        if self.lag_model is None:
            lags = jnp.array([params['%slag%01d' % (self.prefix, i)].value for i in range(len(self.fbins))])
        else:
            lags = self.lag_model(params, self.fbins.bin_cent)

        cov = jnp.sum(jnp.array([p * (c * jnp.cos(phi) - s * jnp.sin(phi)) for p, c, s, phi in zip(cpsd, self.cos_integral, self.sin_integral, lags)]), axis=0)
        return cov
    ...
    def cov_matrix(self, params):
        ...
        return jnp.vstack([jnp.hstack([ac1, cc.T]), jnp.hstack([cc, ac2])])
    ...
    def cross_cov_matrix_deriv(self, params):
        ...
        if self.cpsd_model is None:
            cpsd_derivs = jnp.stack([(c * jnp.cos(phi) - s * jnp.sin(phi)) * p for p, c, s, phi in zip(cpsd, self.cos_integral, self.sin_integral, lags)], axis=-1)
        else:
            psd_model_deriv = self.cpsd_model.eval_gradient(params, self.fbins.bin_cent) * self.psdnorm
            cpsd_derivs = jnp.stack([jnp.sum([pd * (c * jnp.cos(phi) - s * jnp.sin(phi)) for pd, c, s, phi
        ...
        if self.lag_model is None:
            lag_derivs = jnp.stack([-1 * p * (c * jnp.sin(phi) + s * jnp.cos(phi)) for p, c, s, phi in zip(cpsd, self.cos_integral, self.sin_integral, lags)], axis=-1)
        else:
            lag_model_deriv = self.lag_model.eval_gradient(params, self.fbins.bin_cent) * self.psdnorm
            lag_derivs = jnp.stack([jnp.sum([-1 * phid * p * (c * jnp.sin(phi) + s * jnp.cos(phi)) for p, c, s, phi, phid
        ...
        # and (2) the phases
        return jnp.concatenate([cpsd_derivs, lag_derivs], axis=-1)
    ...
    def cov_matrix_deriv(self, params):
        ...
        if self.freeze_psd:
            Z = jnp.zeros_like(self.ac1)
            return jnp.stack(
                [jnp.vstack([jnp.hstack([Z, cc[..., p].T]), jnp.hstack([cc[..., p], Z])]) for p in
            ...
            return jnp.stack(
                [jnp.vstack([jnp.hstack([ac1[..., p], cc[..., p].T]), jnp.hstack([cc[..., p], ac2[..., p]])]) for p in
                 range(len([p for p in params if params[p].vary]))], axis=-1)

        return jnp.stack([jnp.vstack([jnp.hstack([ac1[...,p], cc[...,p].T]), jnp.hstack([cc[...,p], ac2[...,p]])]) for p in range(len(self.params))], axis=-1)
    ...
    def get_cpsd(self, params=None):
        ...
        if self.cpsd_model is None:
            return jnp.array([self.params['%sln_cpsd%01d' % (self.prefix, i)].value for i in range(len(self.fbins))])
        else:
            return jnp.log(self.cpsd_model(self.params, self.fbins.bin_cent))
    ...
    def get_lag(self, params=None, time_lag=True):
        ...
        if self.lag_model is None:
            lag = jnp.array([self.params['%slag%01d' % (self.prefix, i)].value for i in range(len(self.fbins))])
        else:
            lag = self.lag_model(self.params, self.fbins.bin_cent)

        return lag / (2. * jnp.pi * self.fbins.bin_cent) if time_lag else lag
    ...
    def process_fit_results(self, fit_result, params):
        ...
        which are not included in fit_result.x)
        """
        hess = fit_result.hess_inv(fit_result.x) if callable(fit_result.hess_inv) else jnp.diag(fit_result.hess_inv)
        ...
        if self.cpsd_model is None:
            self.lag_error = hess[len(self.fbins):] ** 0.5 / (2. * jnp.pi * self.fbins.bin_cent)
        else:
            return NotImplemented
...
class StackedMLPSD(MLPSD):
    ...
    def __init__(self, lclist, Nf=10, fbins=None, model=None, model_args={}, component_name=None, extend_freq=None, **kwargs):
        if fbins is None:
            ...
            dt = np.min([jnp.min(jnp.diff(lc.time)) for lc in lclist])
            min_freq = (1. / T)
            max_freq = 0.5 / dt
            self.fbins = LogBinning(min_freq, max_freq, Nf)
        else:
    ...
    def log_likelihood(self, params, eval_gradient=True):
        ...
        if eval_gradient:
            ...
            like = jnp.array([l[0] for l in segment_loglike])
            grad = jnp.array([l[1] for l in segment_loglike])
            if jnp.all(jnp.isfinite(like)):
                return jnp.sum(like), grad.sum(axis=0)
            else:
                return (-1e6, jnp.zeros(len(params)) + 1e6)
        else:
            return jnp.array([p.log_likelihood(params, eval_gradient) for p in self.mlpsd]).sum()
...
class StackedMLCrossSpectrum(MLCrossSpectrum):
    ...
    def __init__(self, lclist1, lclist2, Nf=10, fbins=None, cpsd_model=None, lag_model=None, cpsd_model_args={}, lag_model_args={},
        ...
        if fbins is None:
            ...
            dt = np.min([jnp.min(jnp.diff(lc.time)) for lc in lclist1])
            min_freq = (1. / T)
            max_freq = 0.5 / dt
            self.fbins = LogBinning(min_freq, max_freq, Nf)
        else:
    ...
    def log_likelihood(self, params, eval_gradient=True):
        ...
        if eval_gradient:
            ...
            if jnp.all(jnp.isfinite(like)):
                ...
                return (-1e6, jnp.zeros(len([p for p in params if params[p].vary])) - 1e6)
        else:
            return jnp.sum([c.log_likelihood(params, eval_gradient) for c in self.mlcross_spec])
